...
#############################
#balance constrain
@constraint(model, balance[t in 1:Tmax], sum(Pth[t,g] for g in 1:Nth) + sum(Phy[t,h] for h in 1:Nhy) +  [t] + Puns[t] +Pdecharge_battery[t] + Pdecharge_STEP[t] - load[t] - Pexc[t] -Pcharge_STEP[t] - Pcharge_battery[t]== 0)
